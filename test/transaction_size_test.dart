import 'package:blocklib/src/ecpair.dart';
import 'package:blocklib/src/transaction_builder.dart';
import 'package:test/test.dart';
import 'package:blocklib/src/transaction.dart';
import 'package:blocklib/src/models/networks.dart' as NETWORKS;
  ...
  group('Transaction.txSize', () {
    ...
    test('should sign with small signatures', () {
      ...
      for (int i = 0; i < 10; i++) {
        ...
        for (int j = 0; j < 3; j++) txb.addInput(mockHash, j);

        txb.addOutput(mockAddress, BigInt.from(1000 * (i + 1)));

        for (int j = 0; j < 3; j++) txb.sign(vin: j, keyPair: aliceKey);
